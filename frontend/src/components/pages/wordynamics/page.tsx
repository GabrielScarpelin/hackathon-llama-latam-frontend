...
export default function WordDynamicsPage() {
  ...
  const loadNewWord = async () => {
    if (!collectionContext.collection?.words[currentIndex]) return;
    ...
    try {
      if (collectionContext.collection.words[currentIndex].url) {
        ...
        const imageUrl = await handleGenerateImage(
          collectionContext.collection.words[currentIndex].texto_en
        );
  ...
  const handleNext = () => {
    if (isLoading) return;
    if (currentIndex === (collectionContext.collection?.words.length || 1) - 1) {
