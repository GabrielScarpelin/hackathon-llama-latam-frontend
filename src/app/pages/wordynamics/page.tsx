...
export default function Wordynamic() {
  ...
  return (
    <div className="bg-white rounded-3xl p-8 w-full max-w-lg mx-auto flex flex-col h-[500px]">
      ...
      <div className="mb-8">
        <span className="inline-block bg-[#E454A4] text-white px-6 py-2 rounded-full text-sm">
          {isLoading ? 'Carregando...' : currentWord}
        </span>
      </div>
      
      <div className="flex-grow flex items-center mb-6 ml-64">
        {isLoading ? (
            <div className="w-64 h-64 bg-gray-200 rounded-3xl animate-pulse justify-center " />
            ) : currentImage ? (
            <img 
                src={currentImage} 
                alt={currentWord}
        ...
        <div className="w-64 flex justify-end ml-48 ">
            <button 
                onClick={handleNext}
                disabled={isLoading}
                className="bg-[#E454A4] text-white px-6 py-2 rounded-full flex items-center text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#d13d93] transition-colors"
            >
